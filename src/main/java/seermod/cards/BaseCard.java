...
import seermod.SeerMod;
import seermod.util.CardStats;
import seermod.util.TriFunction;
...
import static seermod.util.GeneralUtils.removePrefix;
import static seermod.util.TextureLoader.getCardTextureString;


public abstract class BaseCard extends CustomCard {
    final private static Map<String, DynamicVariable> customVars = new HashMap<>();

    protected static String makeID(String name) { return SeerMod.makeID(name); }
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    SeerMod.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
