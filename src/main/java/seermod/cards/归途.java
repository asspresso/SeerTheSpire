...
public class 归途 extends BaseCard{
    public static final String ID = makeID("Road to Heaven");
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        if (totalBlock >= magicNumber) {
            addToBot(new GainBlockAction(p, p, totalBlock));
        }
        addToBot(new DamageAction(m, new DamageInfo(p, totalBlock, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.NONE));
    }
    ...
    public void triggerOnGlowCheck() {
        ...
        for(AbstractMonster m : AbstractDungeon.getCurrRoom().monsters.monsters) {
            int totalBlock = m.currentBlock;
            if (!m.isDeadOrEscaped() && totalBlock >= magicNumber) {
