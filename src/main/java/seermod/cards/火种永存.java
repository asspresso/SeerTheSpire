...
public class 火种永存 extends BaseCard{
    public static final String ID = makeID("Eternal Fire");
        ...
        if (!AbstractDungeon.getMonsters().areMonstersBasicallyDead()) {
            for(AbstractMonster monster : AbstractDungeon.getMonsters().monsters) {
                if (!monster.isDead && !monster.isDying) {
                    addToBot(new ApplyPowerAction(monster, p, new 烧伤(monster, p, magicNumber), magicNumber));
                }
            }
        }
    }
