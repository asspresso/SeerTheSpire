...
public class 芳馨永存 extends BaseCard{
    public static final String ID = makeID("Eternal Fragrance");
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new MetallicizePower(p, 3), 3));
        if (!AbstractDungeon.getMonsters().areMonstersBasicallyDead()) {
            for(AbstractMonster monster : AbstractDungeon.getMonsters().monsters) {
                if (!monster.isDead && !monster.isDying) {
                    addToBot(new ApplyPowerAction(monster, p, new 中毒(monster, p, magicNumber), magicNumber));
                }
            }
        }
    }
